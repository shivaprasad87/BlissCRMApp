    ...
    function site_visit_report_mail(){
       ...
       /* if ($this->session->userdata('user_type') == 'user') {
        ...
        //    }
           public function previous_callback_apk($id='')
        {
        $indiv_callback_data = $this->callback_model->get_callback_data($this->input->post('callback_id')); 
        $previous_callback = "";
        foreach ($indiv_callback_data as $callback_data) {
            $previous_callback .= $callback_data->status."****".$callback_data->date_added."****".$callback_data->user;
            $previous_callback .= "\n---------------------------------\n";
        ...
        {
            $date_due = str_replace("T"," ",$this->input->post('due_date'));
       ...
       //print_r($data);
        ...
        $query = $this->callback_model->update_callback($update_data,$this->input->post('idoftable'));
...
//echo $this->db->last_query();die;
    ...
    public function thankyou_mail($value='')
    {

        $data['name'] ="thankyou_mail";
        $data['heading'] ="Thankyou Mails";
        $where="";
        $user_type = $this->session->userdata("user_type");
        if($this->input->post()){
            $dept=$this->input->post('dept');
    ...
    public function nextday_calls() {
        $data['name'] ="callbacks";
        $data['heading'] ="Next Day Callbacks";
        $where="";
        $user_type = $this->session->userdata("user_type");

        $where .=" AND cb.due_date >= '".date('Y-m-d  00:00:00', strtotime(' +1 day'))."'";
       $where .=" AND cb.due_date <= '".date('Y-m-d  23:59:59', strtotime(' +1 day'))."'";
       //  $where .=' and cb.due_date <'.date('Y-m-d 23:59:59', strtotime(' +1 day')); 
